...
`api_keys` перечисляет дополнительные ключи того же провайдера. Когда запрос получает ответ о лимите использования (HTTP 429 или 402 при исчерпанном балансе), следующая попытка после обычной задержки повтора идёт со следующим ключом, и остальные запросы этого запуска тоже начинаются с него. Переключение не добавляет запросов: меняется только ключ каждого повтора, а не их число. Другие ошибки, например 401 для отклонённого ключа, ключ не переключают.

Используйте это только с провайдером, который разрешает несколько ключей и ограничивает каждый ключ отдельно. От 429, действующего на весь аккаунт или IP, другой ключ не спасёт.

```bash
ocr config set providers.my-gateway.api_keys "$KEY_1,$KEY_2,$KEY_3"
```

Первым используется `api_key` или, если вместо него задан `api_key_cmd`, вывод этой команды, а затем по порядку `api_keys`. Если не задано ни то, ни другое, первым становится первый элемент `api_keys`. `api_keys` работает как для встроенных, так и для пользовательских провайдеров.

### OpenCode Go

[OpenCode Go](https://opencode.ai/docs/go/) — подписка OpenCode на открытые модели для программирования:
